...
Token = "eyJhbGciOiJFUzM4NCIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiOTUzN2RlMTQtOTAyOC00ZjIxLThkNDQtOTRlMDVkMDRmNzBjIiwidG9rZW5faWQiOiI1MDI2MmNhMC1jNjgwLTRiNTYtYWE0Yi01ZGI1NjliMWYzYzIiLCJyb2xlIjoiT1BFUkFUT1IiLCJleHAiOjE3NzY5NDc0MTV9.Er6hqRAAbdYNJyHw9ZBNiMyVMR9cUazRAgAWS1ZtU23OSYiOWQXx33ULp8UrKFDc-DZB99YN10SqTM1rRZSNbWTMdfF8EKtphr3SICsMu3jeggb-yTl_pNlmInAGGu6m"
...
class Row_BD:
    ...
    def get_raw(self, Token):
        headers = {'x-lamb-auth-token': Token}
        response = requests.get(f'{HOST}/api/chiara/projection_data/{self.projection_id}/{self.row_id}',
                                headers=headers)
        return response.json()
    ...
    def get_raw_filter(self, Token):
        ...
        response = requests.get(f'{HOST}/api/chiara/projection_data/{self.projection_id}/', headers=headers,
                                params=params)
        return response.json()
